import os,re,time,json,random,string,logging,threading
...
async def reg_pass(update,context):
    ...
    if dbc(t,n,g,hp(p)): await update.message.reply_text(f"✅ Регистрация успешна!\n👤 {n}",reply_markup=mk(dbg(t)))
...
async def plat_cb(update,context):
    ...
    if len(lobby["players"])>=LOBBY_SIZE:
        ...
        threading.Timer(READY_CHECK_SECONDS,lambda: asyncio.run_coroutine_threadsafe(_timeout(lobby["id"],context),context.bot.loop)).start()
...
async def winner(update,context):
    ...
    avgw=sum(dbg(p).get("elo",1000) for p in w)/len(w) if w else 0; avgl=sum(dbg(p).get("elo",1000) for p in l)/len(l) if l else 0
    for p in w:
        pp=dbg(p); d=calc(pp["elo"] if pp else 1000,avgw,True,pp.get("matches",0))
        dbup(p,{"elo":max(0,(pp["elo"] if pp else 1000)+d),"wins":(pp.get("wins",0)+1),"matches":(pp.get("matches",0)+1)})
    for p in l:
        pp=dbg(p); d=calc(pp["elo"] if pp else 1000,avgl,False,pp.get("matches",0))
...
def main():
    ...
    app.add_handler(CommandHandler("start",start)); app.add_handler(CommandHandler("help",help)); app.add_handler(CommandHandler("cancel",cancel))
    app.add_handler(CommandHandler("profile",profile)); app.add_handler(CommandHandler("top",top)); app.add_handler(CommandHandler("stats",stats))
    app.add_handler(CommandHandler("premium",premium)); app.add_handler(CommandHandler("promo",promo)); app.add_handler(CommandHandler("support",support))
    app.add_handler(CommandHandler("findmatch",find)); app.add_handler(CommandHandler("winner",winner)); app.add_handler(CommandHandler("addstats",addstats))
    app.add_handler(CommandHandler("admin",admin))
    app.add_handler(ConversationHandler([CommandHandler("register",reg_start)],[REG_NICK:[MessageHandler(filters.TEXT&~filters.COMMAND,reg_nick)],REG_GAMEID:[MessageHandler(filters.TEXT&~filters.COMMAND,reg_gameid)],REG_PASSWORD:[MessageHandler(filters.TEXT&~filters.COMMAND,reg_pass)]},[CommandHandler("cancel",cancel)]))
    app.add_handler(ConversationHandler([CommandHandler("login",login_start)],[LOGIN_NICK:[MessageHandler(filters.TEXT&~filters.COMMAND,login_nick)],LOGIN_GAMEID:[MessageHandler(filters.TEXT&~filters.COMMAND,login_gameid)],LOGIN_PASSWORD:[MessageHandler(filters.TEXT&~filters.COMMAND,login_pass)]},[CommandHandler("cancel",cancel)]))
    ...
    app.add_handler(MessageHandler(filters.TEXT&~filters.COMMAND,lambda u,c: globals().get({"/start":start,"/help":help,"🎮 Найти матч":find,"👤 Профиль":profile,"📊 Статистика":stats,"🏆 Топ игроков":top,"💎 Премиум":premium,"⚙️ Админ-панель":admin}.get(u.message.text,lambda:None)) and globals().get({"/start":start,"/help":help,"🎮 Найти матч":find,"👤 Профиль":profile,"📊 Статистика":stats,"🏆 Топ игроков":top,"💎 Премиум":premium,"⚙️ Админ-панель":admin}.get(u.message.text,lambda:None))(u,c)))
