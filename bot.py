...
async def reg_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⚠️ ВАЖНО! Возвращаем END чтобы выйти из диалога
    return ConversationHandler.END
...
async def changenick_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("✏️ Введите новый ник (2-32 символа, латиница/цифры/_):")
    return REG_NICK
...
async def lobby_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"👥 ИГРОКИ:\n"
        f"1. @{context.user_data.get('nick', 'игрок')} (1000 ELO)\n\n"
        f"⏳ Ожидание: 1/10\n\n"
        f"🚪 Выйти",
        reply_markup=InlineKeyboardMarkup([
...
async def lobby_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("🚪 Вы вышли из лобби.", reply_markup=back_keyboard())
...
async def party_invite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("👤 Введите Telegram юзернейм игрока:", reply_markup=back_keyboard())
...
async def menu_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player:
        await query.edit_message_text("❌ Сначала войдите.", reply_markup=start_keyboard())
        return
    await query.edit_message_text("📝 ИСТОРИЯ МАТЧЕЙ\n\nПока нет сыгранных матчей.", reply_markup=back_keyboard())
