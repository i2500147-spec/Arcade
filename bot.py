...
logger.setLevel(logging.DEBUG)  # <-- Включили DEBUG для деталей
...
async def sb_select(table, params=None):
    try:
        ...
        logger.debug(f"sb_select {table} response: {r.status_code}")
        if r.status_code < 400:
            return r.json()
        logger.error(f"sb_select error: {r.status_code} {r.text}")
        return []
...
async def sb_insert(table, data, upsert=False):
    try:
        ...
        logger.debug(f"sb_insert {table} response: {r.status_code}")
        if r.status_code < 400:
            return r.json()
        logger.error(f"sb_insert error: {r.status_code} {r.text}")
        return []
    except Exception as e:
        logger.error(f"sb_insert exception: {e}")
        return []
...
def admin_approve_kb(pending_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Впустить", callback_data=f"approve_{pending_id}"),
         InlineKeyboardButton("❌ Отказать", callback_data=f"reject_{pending_id}")]
    ])
...
async def cb_check_sub(update, context):
    ...
    if await check_subscription(context, q.from_user.id):
        await q.edit_message_text("Спасибо за подписку! Выберите действие:", reply_markup=entry_kb())
    else:
...
async def handle_register_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"📝 Сохранение заявки: telegram_id={user.id}, game_id={data['game_id']}, nick={data['nick']}")
    ...
    if not result:
        ...
        await update.message.reply_text("❌ Ошибка сохранения заявки. Попробуйте позже.")
        return

    clear_state(user.id)
    await update.message.reply_text("✅ Заявка отправлена админам.")

    if ADMIN_CHAT_ID and result:
        pending_id = result[0].get("id")
        caption = (
        ...
        await context.bot.send_photo(
            ...
            photo_id,
            caption=caption,
            reply_markup=admin_approve_kb(pending_id),
        )
...
async def cb_approve_registration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_id = int(q.data.split("_")[1])
    
    pending = await sb_select("pending", {"id": f"eq.{pending_id}", "limit": "1"})
    if not pending:
    ...
    if existing:
        await sb_delete("pending", {"id": f"eq.{pending_id}"})
        await q.edit_message_caption(caption=(q.message.caption or "") + "\n\n✅ Уже зарегистрирован")
        return
