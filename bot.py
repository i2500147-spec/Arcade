...
Версия: 3.4 (с Supabase вместо JSON)
"""
...
REQUIRED_ENV = ["BOT_TOKEN", "ADMIN_IDS", "GENERAL_CHAT_ID", "ADMIN_CHAT_ID", "SUPABASE_URL", "SUPABASE_KEY"]
...
def init_supabase():
    """Создаёт таблицы в Supabase, если их нет"""
    try:
        # Проверяем наличие таблицы players
        supabase.table('players').select('*').limit(1).execute()
        logger.info("✅ Таблицы существуют")
    except Exception:
        logger.warning("⚠️ Создаю таблицы в Supabase...")
        try:
            ...
            for statement in sql.split(';'):
                if statement.strip():
                    try:
                        ...
                        logger.warning(f"Не удалось выполнить SQL: {e}")
...
def load_players():
    try:
        ...
        for row in result.data:
            data[row['user_id']] = row['data']
        return data
    except Exception as e:
        logger.error(f"Ошибка загрузки players: {e}")
        return {}

def save_players(p):
    try:
        supabase.table('players').delete().neq('user_id', 'null').execute()
        for user_id, data in p.items():
            supabase.table('players').insert({'user_id': user_id, 'data': data}).execute()
    except Exception as e:
        logger.error(f"Ошибка сохранения players: {e}")

def load_pending():
    try:
        result = supabase.table('pending').select('*').execute()
        data = {}
        for row in result.data:
...
def save_pending(p):
    try:
        supabase.table('pending').delete().neq('id', 'null').execute()
        for pid, data in p.items():
            supabase.table('pending').insert({'id': pid, 'data': data}).execute()
    except Exception as e:
        logger.error(f"Ошибка сохранения pending: {e}")

def load_lobbies():
    try:
        result = supabase.table('lobbies').select('*').execute()
        data = {}
        for row in result.data:
            data[row['platform']] = row['lobby_data']
        return data
    except Exception:
        return {"Phone": [[], [], [], [], [], []], "PC": [[], [], [], [], [], []]}
...
def save_lobbies(l):
    try:
        supabase.table('lobbies').delete().neq('platform', '').execute()
        for platform, lobby_data in l.items():
            supabase.table('lobbies').insert({'platform': platform, 'lobby_data': lobby_data}).execute()
    except Exception as e:
        logger.error(f"Ошибка сохранения lobbies: {e}")

def load_parties():
    try:
        result = supabase.table('parties').select('*').execute()
        data = {}
        for row in result.data:
            data[row['leader_id']] = row['data']
        return data
    except Exception:
        return {}
...
def save_parties(p):
    try:
        supabase.table('parties').delete().neq('leader_id', 'null').execute()
        for leader_id, data in p.items():
            supabase.table('parties').insert({'leader_id': leader_id, 'data': data}).execute()
    except Exception as e:
        logger.error(f"Ошибка сохранения parties: {e}")
    ...
    try:
        ...
        for row in result.data:
            ...
            if target not in data:
                data[target] = []
            data[target].append(row['report_data'])
        return data
    except Exception:
        return {}
...
def save_reports(r):
    try:
        ...
        for target_uid, reports in r.items():
        ...
        logger.error(f"Ошибка сохранения reports: {e}")

def load_history():
    try:
        result = supabase.table('match_history').select('*').order('id', desc=True).limit(100).execute()
        return {"matches": [row['data'] for row in result.data]}
    except Exception:
        return {"matches": []}
...
def save_history(h):
    try:
        ...
        for entry in h.get('matches', []):
            supabase.table('match_history').insert({'data': entry}).execute()
    except Exception as e:
...
def append_history(entry):
    try:
        ...
        logger.error(f"Ошибка append_history: {e}")

def load_analytics():
    try:
        result = supabase.table('analytics').select('*').execute()
        data = {"map_picks": {}, "online_samples": [], "match_timestamps": []}
        for row in result.data:
            data[row['key']] = row['value']
        return data
    except Exception:
        return {"map_picks": {}, "online_samples": [], "match_timestamps": []}

def save_analytics(a):
    try:
        supabase.table('analytics').delete().neq('key', '').execute()
        for key, value in a.items():
            supabase.table('analytics').insert({'key': key, 'value': value}).execute()
    except Exception as e:
        logger.error(f"Ошибка сохранения analytics: {e}")

# ===================== ИГРОКИ И ДРУГИЕ ФУНКЦИИ =====================
...
def is_premium(player) -> bool:
    if not player:
        return False
    premium_until = player.get('premium_until', 0)
    return premium_until > time.time()

def get_premium_time_left(player) -> int:
    if not player:
        return 0
    premium_until = player.get('premium_until', 0)
    if premium_until <= time.time():
        return 0
    return int(premium_until - time.time())

def format_premium_time(seconds: int) -> str:
    if seconds <= 0:
        return "не активен"
    days = seconds // 86400
...
def elo_display(player):
    ...
    premium = "💎" if is_premium(player) else ""
    return f"{player['rank']} {premium}• {player['elo']} ELO"
...
def update_lobby_for_all(platform: str, lobby_idx: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in players_list:
        ...
        elo = p.get("elo", 0)
        premium = "💎" if is_premium(p) else ""
        lines.append(f"@{tag} {premium}({elo} ELO)")
    text = f"📋 ЛОББИ {lobby_idx + 1} ({len(players_list)}/{LOBBY_SIZE})\n\nИГРОКИ:\n"
    for i, p in enumerate(lines, 1):
        text += f"{i}. {p}\n"
    text += f"\nОжидание: {len(players_list)}/{LOBBY_SIZE}"
    for uid in players_list:
        safe_send(context.bot, uid, text, reply_markup=kb_in_lobby(platform, lobby_idx))
...
def kb_premium_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⭐ 1 день — 50 звёзд", callback_data="premium:day")],
...
def main_menu_text(player):
    ...
    premium_status = "💎 Премиум активен!" if is_premium(player) else ""
    return (
        f"🏠 ГЛАВНОЕ МЕНЮ\n\n"
        f"👤 @{tag}\n"
        f"📊 {elo}\n"
...
def profile_text(player):
    ...
    premium_text = "💎 Премиум активен!" if is_premium(player) else "Нет премиума"
    if is_premium(player):
        time_left = get_premium_time_left(player)
        premium_text += f" (осталось {format_premium_time(time_left)})"
...
def party_text(parties, leader_id, players):
    ...
    for uid in party["members"]:
        ...
        premium = "💎" if is_premium(p) else ""
        lines.append(f"{crown}@{tag} {premium}")
    ...
    try:
        ...
        if data == "menu:premium":
            ...
            if not player or player.get("reg") != 1:
                await query.message.reply_text("Сначала зарегистрируйся!", reply_markup=kb_start_auth())
                return
            await query.message.reply_text(premium_text(), reply_markup=kb_premium_menu(), parse_mode="Markdown")
            return
        ...
        if data.startswith("premium:"):
            ...
            duration = PREMIUM_DURATIONS[period]
            period_names = {"day": "1 день", "week": "1 неделя", "month": "1 месяц"}
            
            await safe_delete(query.message)
            
            # Отправляем счёт на оплату звёздами
            await context.bot.send_invoice(
                chat_id=user.id,
                title=f"Премиум-подписка {period_names[period]}",
                description=f"x2 ELO, любой тег, бесплатные турниры",
                payload=f"premium_{period}",
                currency="XTR",
                prices=[{"label": period_names[period], "amount": price}],
                start_parameter=f"premium_{period}",
            )
            return
        ...
        if user.id == OWNER_ID and data.startswith("admin:"):
            ...
            if action == "give_tag":
                ...
                await query.message.reply_text(
                    "🏷️ ВЫДАТЬ ТЕГ\n\nВыбери игрока:",
                    reply_markup=kb_players_list(all_players, "give_tag", 0, back_cb="admin:back"),
                )
                return
        ...
        for prefix in ["unlink", "ban", "eloact_add", "eloact_remove", "chg_change_id", "chg_change_nick", "give_tag"]:
            if data.startswith(f"{prefix}_page:"):
                ...
                key_map = {
                    ...
                    "give_tag": "admin_give_tag_players",
                }
        ...
        if data.startswith("give_tag:") and user.id == OWNER_ID:
            ...
            await safe_delete(query.message)
            await query.message.reply_text(
                f"🏷️ Введи новый тег для @{target_p.get('tag', target_uid)}:"
            )
            context.user_data['admin_input_mode'] = 'give_tag'
            return
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if period not in PREMIUM_DURATIONS:
        return
    ...
    player = get_player(players, user_id)
    if player:
        new_until = int(time.time()) + duration
        player['premium_until'] = new_until
        ...
        await update.message.reply_text(
            ...
            f"• x2 ELO\n"
            f"• Любой тег\n"
            f"• Бесплатные турниры\n"
            f"• Смена ника (2 раза бесплатно, далее 50⭐)"
        )
    else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('admin_input_mode') == 'give_tag' and user.id == OWNER_ID:
            target_uid = context.user_data.get('admin_give_tag_target')
            fresh_players = load_players()
            target_player = fresh_players.get(target_uid)
            if not target_player:
                await update.message.reply_text("❌ Игрок не найден.")
            ...
            # Разрешаем ЛЮБОЙ тег, даже занятый
            old_tag = target_player['tag']
            target_player['tag'] = text
            target_player['name'] = text
            audit_log("admin_give_tag", user.id, {"target": target_uid, "old": old_tag, "new": text})
            save_players(fresh_players)
            await update.message.reply_text(f"✅ Тег @{old_tag} изменён на: {text}")
            context.user_data.pop('admin_input_mode', None)
            context.user_data.pop('admin_give_tag_target', None)
            return
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 Stranger Faceit 3.4 запущен!")
