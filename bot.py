...
Версия: 3.0 (вход/регистрация, расширенная статистика, жалобы, аналитика, админ-панель)
Всё в одном файле. Без Pillow. Готов к деплою на Render (free tier).
"""
...
def load_analytics():
    return _load_json(ANALYTICS_FILE, {
        "map_picks": {}, "online_samples": [], "match_timestamps": [],
    })
...
def kb_reg_done():
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ Сделал", callback_data="auth:reg_done")]])

def kb_stats_extended():
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("⬅️ Назад", callback_data="menu:main"),
        InlineKeyboardButton("🔄 Обновить", callback_data="menu:stats"),
    ]])
...
async def check_banned(update: Update) -> bool:
    ...
    if is_banned(player):
        await update.effective_message.reply_text("⛔ Вы забанены и не можете пользоваться ботом.")
        return True
    return False
...
async def require_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if REQUIRE_SUBSCRIPTION and not await is_subscribed(context.bot, update.effective_user.id):
        await update.effective_message.reply_text(
            "📢 Подпишись на наш чат, чтобы использовать бота!", reply_markup=kb_subscribe(),
        )
        return False
    return True
...
async def update_lobby_for_all(platform: str, lobby_idx: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in players_list:
        await safe_send(context.bot, uid, text, reply_markup=kb_in_lobby(platform, lobby_idx), parse_mode="Markdown")
...
def extended_stats_text(player):
    ...
    text = (
        ...
        f"🔫 AVG KD: {avg_kd}\n"
        f"🎯 HS%: {hs_pct}%\n"
        f"🗺️ Любимая карта: {fav_map_text}\n\n"
...
def complaints_players_text():
    return "📢 *ЖАЛОБЫ*\n\nВыбери игрока, чтобы написать или посмотреть жалобы:"
...
def build_analytics_text(players, pending):
    ...
    return (
        f"📊 *АНАЛИТИКА*\n\n"
        f"🗺️ Самая выбираемая карта: {MAP_EMOJI.get(top_map, '')} {top_map}\n"
        f"👥 Средний онлайн: {avg_online}\n"
        f"📈 Средний ELO: {avg_elo}\n"
        f"⏰ Пиковое время (МСК): {peak_text}\n"
        f"🎮 Активные матчи на проверке: {active_matches}\n\n"
        f"📢 Топ жалоб по категориям:\n{top_categories_text}"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player or player.get("reg") != 1:
        await update.message.reply_text(
            ...
            reply_markup=kb_start_auth(), parse_mode="Markdown",
        )
        return
    ...
    await update.message.reply_text(
        main_menu_text(player), reply_markup=kb_main_menu(in_party=bool(leader_id)), parse_mode="Markdown",
    )
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        ...
        return
    await update.message.reply_text("👑 *АДМИН-ПАНЕЛЬ*\n\nВыбери действие:", reply_markup=kb_admin_panel(), parse_mode="Markdown")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match.get('host') != update.effective_user.id:
        await update.message.reply_text("❌ Отправлять результаты может только хост матча.")
        return
    if time.time() < match.get('result_unlock_time', 0):
        remaining = int(match['result_unlock_time'] - time.time())
        await update.message.reply_text(f"⏳ Отправка результатов будет доступна через {remaining} сек.")
        return
...
async def winner_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not match or match.get('status') != 'in_progress':
        ...
        return
    if match.get('host') != update.effective_user.id:
        await update.message.reply_text("❌ Объявлять результат может только хост матча.")
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ---------- ВХОД / РЕГИСТРАЦИЯ / ПОДДЕРЖКА (стартовое меню) ----------
        if data == "auth:login":
            await safe_delete(query.message)
        ...
        if data == "auth:reg_done":
            ...
            if not pending_sid or not pending_tag:
                await query.answer("❌ Сессия регистрации истекла. Начни заново.", show_alert=True)
                return
            fresh_players = load_players()
            if find_by_sid(fresh_players, pending_sid) or find_by_tag(fresh_players, pending_tag):
                await query.answer("❌ Этот ID или ник уже заняты. Начни заново.", show_alert=True)
                context.user_data.pop('auth_step', None)
                return
            new_p = new_player(pending_sid, user.username or str(user.id))
            new_p["reg"] = 1
            new_p["name"] = pending_tag
            new_p["tag"] = pending_tag
            fresh_players[user_id] = new_p
            save_players(fresh_players)
            invalidate_cache()
            context.user_data.pop('auth_step', None)
            context.user_data.pop('reg_pending_sid', None)
            context.user_data.pop('reg_pending_tag', None)
            await safe_delete(query.message)
            await query.message.reply_text(
        ...
        if data == "menu:main":
            ...
            if not player or player.get("reg") != 1:
                await query.message.reply_text(
                    "🎮 *STRANGER FACEIT*\n\nВыбери действие:", reply_markup=kb_start_auth(), parse_mode="Markdown",
                )
                return
            parties = load_parties()
            leader_id, _ = find_party_of(parties, user.id)
        ...
        if data == "menu:stats":
            ...
            if not player or player.get("reg") != 1:
                await query.message.reply_text("Сначала зарегистрируйся!", reply_markup=kb_start_auth())
                return
            await query.message.reply_text(extended_stats_text(player), reply_markup=kb_stats_extended(), parse_mode="Markdown")
            return

        # ---------- ИСТОРИЯ МАТЧЕЙ (личная) ----------
        if data == "menu:history":
            ...
            await query.message.reply_text(
                personal_history_text(user_id, history), reply_markup=kb_back_main(), parse_mode="Markdown",
            )
            return
        ...
        if data.startswith("complaint_write:"):
            ...
            if target_uid == user_id:
                await query.answer("❌ Нельзя пожаловаться на самого себя.", show_alert=True)
                return
            reports = load_reports()
            existing = reports.get(target_uid, [])
            if any(r.get("by") == user_id for r in existing):
                await query.answer("❌ Ты уже подавал(а) жалобу на этого игрока.", show_alert=True)
                return
            context.user_data['complaint_target'] = target_uid
            await safe_delete(query.message)
            await query.message.reply_text(
                f"✍️ Опиши жалобу на @{players.get(target_uid, {}).get('tag', target_uid)} одним сообщением "
                f"(до {MAX_REPORT_LEN} символов):",
        ...
        if data.startswith("veto_ban:"):
            ...
            if veto["final_map"]:
                ...
                final_text = (
                    ...
                    f"⏱️ Раунд: 1:50\n"
                    f"🖥️ Хост: @{host_p.get('tag', match.get('host'))}\n\n"
                    f"📌 После матча хост отправляет фото результата, затем:\n`/winner ct` или `/winner t`"
                )
        ...
        if data.startswith("winteam:"):
            ...
            if match.get('host') != user.id:
                await query.answer("❌ Только хост может указать команду-победителя.", show_alert=True)
                return
            ...
            await query.message.reply_text(
                ...
                "Например:\n`@Vasya 18-9-5`\n\nОтправляй по одному игроку за сообщение, либо пропусти статистику.",
        ...
        if user.id == OWNER_ID and data.startswith("admin:"):
            ...
            if action == "complaints_top":
                ...
                await query.message.reply_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="admin:back")]]), parse_mode="Markdown")
                return
            ...
            if action == "history":
                ...
                await safe_delete(query.message)
                await query.message.reply_text(text, reply_markup=kb_history_nav(0, total_pages), parse_mode="Markdown")
                return
            ...
            if action == "unlink":
                ...
                await query.message.reply_text(
                    "🆔 *ОТВЯЗАТЬ АЙДИ*\n\nВыбери игрока:",
                    reply_markup=kb_players_list(all_players, "unlink", 0, back_cb="admin:back"), parse_mode="Markdown",
                )
                return
            ...
            if action == "ban":
                ...
                await query.message.reply_text(
                    "🔨 *ЗАБАНИТЬ ИГРОКА*\n\nВыбери игрока:",
                    reply_markup=kb_players_list(all_players, "ban", 0, back_cb="admin:back"), parse_mode="Markdown",
                )
                return

            if action == "elo":
                await safe_delete(query.message)
                await query.message.reply_text("📊 *УПРАВЛЕНИЕ ELO*\n\nВыбери действие:", reply_markup=kb_admin_elo_action(), parse_mode="Markdown")
                return
        ...
        if data.startswith("admin_history_page:"):
            ...
            await query.message.edit_text(text, reply_markup=kb_history_nav(page, total_pages), parse_mode="Markdown")
            return

        # -- пагинация списков игроков (общая) --
        ...
        if data.startswith("unlink_yes:") and user.id == OWNER_ID:
            ...
            if target_p:
                ...
                save_players(fresh_players)
                await safe_send(context.bot, int(target_uid), "🆔 Твой аккаунт был отвязан администратором. Вся статистика сброшена.")
            await safe_delete(query.message)
        ...
        if data.startswith("ban_yes:") and user.id == OWNER_ID:
            ...
            if target_p:
                ...
                await safe_send(context.bot, int(target_uid), "🔨 Ты был(а) забанен(а) администратором.")
            await safe_delete(query.message)
...
async def _notify_result_ready(host_id, match_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_send(
        ...
        reply_markup=kb_send_results(), parse_mode="Markdown",
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('admin_input_mode') in ('change_id', 'change_nick') and user.id == OWNER_ID:
            ...
            if mode == 'change_id':
                ...
                if find_by_sid(fresh_players, text):
                    await update.message.reply_text("❌ Этот ID уже занят другим игроком!")
                    return
        ...
        if context.user_data.get('auth_step') == 'login_id':
            if not validate_standoff_id(text):
                await update.message.reply_text("❌ ID должен быть числом из 8-15 цифр!", reply_markup=kb_back_main())
                return
            owner_uid = find_by_sid(players, text)
            if not owner_uid:
                await update.message.reply_text("❌ Игрок с таким ID не найден. Проверь ID или зарегистрируйся.", reply_markup=kb_back_main())
                return
            context.user_data['login_sid'] = text
            context.user_data['login_owner_uid'] = owner_uid
            context.user_data['auth_step'] = 'login_name'
            await update.message.reply_text("✅ ID найден!\n\nТеперь введи свой *ник в Standoff 2*:", reply_markup=kb_back_main(), parse_mode="Markdown")
            return
        ...
        if context.user_data.get('auth_step') == 'login_name':
            ...
            if not owner_p:
                await update.message.reply_text("❌ Ошибка. Начни заново.", reply_markup=kb_back_main())
                context.user_data.clear()
                return
            ...
            await update.message.reply_text("✅ Вход выполнен!", parse_mode="Markdown")
            parties = load_parties()
            leader_id, _ = find_party_of(parties, user.id)
            await update.message.reply_text(main_menu_text(player), reply_markup=kb_main_menu(bool(leader_id)), parse_mode="Markdown")
            return
        ...
        if context.user_data.get('auth_step') == 'reg_id':
            ...
            if find_by_sid(players, text):
                ...
                return
            tag = gen_unique_tag(players)
            context.user_data['reg_pending_sid'] = text
            context.user_data['reg_pending_tag'] = tag
            context.user_data['auth_step'] = None
            await update.message.reply_text(
                f"✅ ID принят!\n\nТеперь поставь себе этот ник в Standoff 2:\n\n`{tag}`\n\n"
                f"После этого нажми кнопку «Сделал».",
                reply_markup=kb_reg_done(), parse_mode="Markdown",
            )
            return
        ...
        if context.user_data.get('party_invite_mode'):
            ...
            await update.message.reply_text(
                f"✅ Приглашение отправлено игроку @{target_player.get('tag', target_uid)}!", reply_markup=kb_back_main(),
            )
            ...
            if sent is None:
                await update.message.reply_text("⚠️ Не удалось доставить приглашение (у игрока закрыты ЛС с ботом).")
            return
        ...
        if context.user_data.get('support_mode'):
            ...
            msg = f"🆘 *Поддержка*\n\n👤 @{tag} (ID: {user_id})\n📝 Сообщение:\n{text}"
            await safe_send(context.bot, ADMIN_CHAT_ID, msg, parse_mode="Markdown")
            ...
            if player and player.get("reg") == 1:
                ...
                await update.message.reply_text("✅ Запрос отправлен администраторам!", reply_markup=kb_start_auth())
            return
        ...
        if context.user_data.get('complaint_target'):
            ...
            if any(r.get("by") == user_id for r in reports[target_uid]):
                await update.message.reply_text("❌ Ты уже подавал(а) жалобу на этого игрока.", reply_markup=kb_back_main())
                return
            reports[target_uid].append({"by": user_id, "text": report_text, "timestamp": datetime.now().isoformat()})
            save_reports(reports)
            target_p = players.get(target_uid, {})
            await update.message.reply_text(f"✅ Жалоба на @{target_p.get('tag', target_uid)} отправлена.", reply_markup=kb_back_main())
            return
...
async def handle_stats_input(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if not match:
        ...
        return
    if match.get('host') != update.effective_user.id:
        await update.message.reply_text("❌ Вводить статистику может только хост.")
        return

    m = STATS_LINE_RE.match(text)
    if not m:
        await update.message.reply_text(
            "❌ Неверный формат. Используй: `@ник убийства-смерти-хс` (хс необязательно)\nНапример: `@Vasya 18-9-5`",
            reply_markup=kb_skip_stats(), parse_mode="Markdown",
        )
        return
...
async def finalize_match(update: Update, context: ContextTypes.DEFAULT_TYPE, skip_stats: bool):
    ...
    if target_chat:
        ...
        await safe_send(
            ...
            f"📋 *Матч на проверку*\n🆔 *{match_id}*\n📍 Карта: *{map_name}*\n\n"
            f"*🔵 ПОБЕДА:*\n{winners_text}\n\n*🔴 ПОРАЖЕНИЕ:*\n{losers_text}",
            parse_mode="Markdown", reply_markup=kb_admin_review(pending_id),
        )
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 Stranger Faceit 3.0 запущен!")
    logger.info(f"👑 Админы: {ADMIN_IDS}")
    logger.info(f"👑 Владелец: {OWNER_ID}")
    logger.info(f"🏠 Общий чат: {GENERAL_CHAT_ID}")
    logger.info(f"🔒 Админ-чат: {ADMIN_CHAT_ID}")
    logger.info(f"📁 Данные: {DATA_FILE}, {PENDING_FILE}, {LOBBIES_FILE}, {PARTIES_FILE}, {REPORTS_FILE}, {HISTORY_FILE}, {ANALYTICS_FILE}")
    logger.info("=" * 50)
