...
def promo_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⬅️ Назад", callback_data="premium_menu")],
    ])
...
async def menu_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"🔫 AVG K/D: {avg_kd}\n"
        f"🎯 HS%: {hs_pct}%\n"
        f"🗺️ Любимая карта: {player.get('fav_map', '—')}"
    )
    await query.edit_message_text(text, reply_markup=stats_keyboard(), parse_mode="Markdown")
...
async def menu_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(text, reply_markup=premium_keyboard(), parse_mode="Markdown")
    ...
    if ADMIN_CHAT_ID:
        ...
        try:
            await context.bot.send_message(
                ...
                f"👤 От: @{SUPPORT_TICKETS[ticket_id]['username']} (ID: {user.id})\n\n"
                f"📝 Сообщение:\n{update.message.text}",
                reply_markup=kb,
                parse_mode="Markdown"
            )
            logger.info(f"Тикет {ticket_id} отправлен в админ-чат {ADMIN_CHAT_ID}")
        except Exception as e:
            logger.error(f"Ошибка отправки тикета в админ-чат: {e}")
    else:
        logger.warning("ADMIN_CHAT_ID не задан!")

    await update.message.reply_text(
        "✅ Сообщение отправлено в админ-чат. Мы ответим вам здесь.",
        reply_markup=main_menu_keyboard()
    )
    return ConversationHandler.END
...
async def admin_reply_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, что ответ из АДМИН-ЧАТА
    if update.effective_chat.id != ADMIN_CHAT_ID:
        await query.answer("❌ Только из админ-чата!", show_alert=True)
        return
...
async def admin_reply_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"Ответ на тикет {ticket_id} отправлен пользователю {ticket['user_id']}")
    except Exception as e:
...
async def changenick_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "✏️ Введите новый ник (2-32 символа, латиница/цифры/_):"
    )
    return REG_NICK
...
async def platform_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = []
    for i in range(1, 7):
        kb.append([InlineKeyboardButton(f"Лобби {i} (0/10)", callback_data=f"lobby_{platform}_{i}")])
...
async def lobby_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, platform, lobby_id = query.data.split("_")
    player = get_player_by_tg(query.from_user.id)

    await query.edit_message_text(
        f"📋 **ЛОББИ {lobby_id}** (1/10)\n\n"
        f"👥 **ИГРОКИ:**\n"
        f"1. @{player['nick'] if player else 'игрок'} (1000 ELO)\n\n"
        f"⏳ Ожидание: 1/10",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🚪 Выйти", callback_data=f"lobby_leave_{platform}_{lobby_id}")]
        ]),
        parse_mode="Markdown"
    )

async def lobby_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def menu_party(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Пригласи друга!",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ Пригласить", callback_data="party_invite")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back")],
        ]),
        parse_mode="Markdown"
    )
...
async def admin_reset_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "👤 Выберите игрока для сброса пароля:",
        reply_markup=InlineKeyboardMarkup(kb)
    )
...
async def admin_reset_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        "🔑 Введите новый пароль (мин. 8 символов, латиница+цифры):"
    )
    return RESET_PASS
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(platform_select, pattern="^platform_"))
    app.add_handler(CallbackQueryHandler(lobby_join, pattern="^lobby_"))
    app.add_handler(CallbackQueryHandler(lobby_leave, pattern="^lobby_leave_"))
